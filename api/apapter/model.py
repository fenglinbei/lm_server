...
class BaseModelAdapter:
    ...
    def load_model(self, model_name: Optional[str] = None, model_dir: Optional[str] = None, adapter_model: Optional[str] = None, **kwargs):
        ...
        if adapter_model is not None:
            try:
                ...
                tokenizer = self.tokenizer_class.from_pretrained(model_name, **tokenizer_kwargs)
        else:
            if "ggml" in model_name:
                tokenizer = self.tokenizer_class.from_pretrained(model_dir, cache_dir=model_dir, **tokenizer_kwargs)
            else:
                tokenizer = self.tokenizer_class.from_pretrained(model_name, cache_dir=model_dir, **tokenizer_kwargs)
            
        if "ggml" in model_name:
            import chatglm_cpp
            model = chatglm_cpp.Pipeline(model_name, dtype="q4_0")
        ...
        if kwargs.get("device_map", None) == "auto":
            config_kwargs["device_map"] = "auto"
        config = AutoConfig.from_pretrained(model_name, cache_dir=model_dir, trust_remote_code=True)
...
class Qwen14BModelAdapter(BaseModelAdapter):
    """ https://github.com/QwenLM/Qwen-7B """

    model_names = ["Qwen/Qwen-14B-Chat-Int4"]

    @property
    def default_model_name_or_path(self):
        return "Qwen/Qwen-14B-Chat-Int4"
...
register_model_adapter(Qwen14BModelAdapter)
